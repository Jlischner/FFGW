program diel
  ...
  REAL(DP) :: G2max, Vcell,dV,pi,absq,matfac,Rc,alat,eta_ev
  ...
  REAL(DP), ALLOCATABLE, DIMENSION(:,:) :: G,Gact,eigs,wf1,wf2,wfq1,wfq2,gvecs
  ...
  OPEN(10,FILE="ffgw.inp",form='formatted')
  READ(10,*) S(1),S(2),S(3)
  READ(10,*) alat
  READ(10,*) nprow
  READ(10,*) G2max
...
!  S(1) = 96; S(2) = 96; S(3) = 96
  ...
  R(1,1) = alat;  R(1,2) =  0.0;  R(1,3) =  0.0
  R(2,1) =  0.0;  R(2,2) = alat;  R(2,3) =  0.0
  R(3,1) =  0.0;  R(2,3) =  0.0;  R(3,3) = alat 
...
!  nbands = 100
  ...
  wfblocksize = 2 !how many valence states per proc
  blocksize = 64 !blocksize for scalapack
  ...
  K   = 4.*pi / Gact2 * (1 - cos( sqrt(G2)*Rc))
  K(1)= 2.*pi*Rc**2  

  KX    = 4.*pi /G2 * (1 - cos( sqrt(G2)*Rc))
  KX(1) = 2.*pi*Rc**2
  ...
  if(iam == master) then
     ...
     ALLOCATE( gvecs(Ngk,3) , STAT=error)
     IF (error /= 0) THEN
        PRINT *, "could not allocate space for gvecs"; STOP
     END IF   
     ...
     OPEN(10,FILE=fil_gkv,form='formatted')
     DO i=1,Ngk
        READ(10,*) gvecs(i,1),gvecs(i,2),gvecs(i,3)
     END DO
     CLOSE(10)

     call get_fftbox(gvecs,S,Ngk,indx)
     open(10,file="indx_out",form='formatted')
     do i=1,Ngk
        write(10,*) indx(i)
     enddo
     close(10)
  ...
  call MPI_BCAST(de,2*npair,MPI_REAL,master,MPI_COMM_WORLD,ierr) 
  ...
  if(iam < nprocsNew) then
     ...
     DO iv = nv_start, nv_start+nv_len-1 
        ...
        DO ic=1,nc(is)
           ...
           lqRem(Ind) =  0.0*dV* SUM( CONJG(wfc) * wfvq ) /absq ;
           Ind = Ind + 1
        END DO
     END DO
  ...
  !IV_new = 9
  !IS_new = 1
  !Ieta  = cmplx( 0.0, 0.01/27.21)
  Ieta  = cmplx( 0.0, eta_ev/27.21)
  !nfreq = 200
  !ws_start = -20.0 !eV
  !ws_step  = 0.1 !eV
  ...
  if(iam == master) then
     ...
     !self consistency:
     !eigs_new(9,1) = -10.91/27.21
     !eigs_new(6,1) = eigs_new(7,1)
     !eigs_new(2,1) = eigs_new(4,1)

     if(iam == master) then
        print *, "using ionization potential=",eigs_new(nvu,1)*27.21
     end if
